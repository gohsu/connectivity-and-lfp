...
```{r desc2, echo=FALSE}
iv_colnames <- c("pca1_stock", "med_hh_income_1000", "avg_rooms_per_dwelling", "percent_hh_with_children", "lfp_male", "percent_drivers_female", "percent_publictransit_female")
csd <- aggregate(df_no_geom$pca1_stock, by=list(df$csd_uid), mean)
colnames(csd) <- c('csd_uid', 'pca1_stock')
avg_sndi <- mean(csd$pca1_stock)
ggplot(csd, aes(pca1_stock)) + geom_histogram(color="black", fill="white") + geom_vline(xintercept=avg_sndi, color='red') + labs(title='Street network disconnectedness in Canada by municipality', x='SNDI') 
```
...
plot(mtl_data['lfp_gap'], breaks='quantile', main="LFP Gap in Montreal")
...
### Toronto CSD
```{r lfp_to, echo=FALSE }
plot(to_data['lfp_gap'], breaks='quantile', main="LFP Gap in Toronto")
